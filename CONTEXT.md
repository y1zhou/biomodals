...
produces a new cumulative result while leaving the source result unchanged.
_Avoid_: failed-work retry, rerun from the initial structure, Result preparation

**Continuation Source**:
The completed simulation whose final dynamical state and production history
are inherited by a Production Continuation.
_Avoid_: initial PDB, final structure alone, failed Execution Run predecessor

### Antibody humanization

**Antibody Variable Region**:
The amino acid sequence of one antibody heavy- or light-chain variable domain.
_Avoid_: full antibody, antibody chain
...
Antibody Variable Region. It is neither an OASis humanness score nor a
calibrated immunogenicity estimate.
_Avoid_: humanness score, immunogenicity score, OASis score

**Sapiens Humanization**:
A transformation of an Antibody Variable Region that replaces every eligible
...
the default for Sapiens Humanization.
_Avoid_: numbering scheme, implicit residue range

**Antibody Numbering Scheme**:
A named coordinate system for assigning antibody positions and insertion
codes. It is selected independently from the CDR Definition.
_Avoid_: CDR Definition, raw sequence offset

**Parental CDR Preservation**:
The Sapiens Humanization policy that restores the parental amino acid at every
position classified as a CDR after each iteration. CDR mutation is a distinct,
explicit opt-in policy.
_Avoid_: model confidence threshold, Vernier back-mutation

**Humanization Iteration**:
One complete Sapiens Humanization pass over an Antibody Variable Region. Each
pass consumes the preceding pass's sequence.
_Avoid_: convergence loop, stochastic sample, execution retry

**Sapiens Endpoint Scores**:
The Sapiens Residue Score matrices for the parental Antibody Variable Region
and its final humanized sequence. Intermediate Humanization Iteration scores
are not part of the scientific result.
