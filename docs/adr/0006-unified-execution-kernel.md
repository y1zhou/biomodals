...
remaining frontier and no Provider Call is active. Workloads may still use a
zero GPU ceiling internally for CPU-only Runs. The unified CLI accepts only
positive explicit `--max-gpu-containers` values; omit the option when a
CPU-only workload should use its own default. The kernel adds no active-image
heuristic, fairness cursor, priority weights, preemption, or scheduler plugin
surface.
