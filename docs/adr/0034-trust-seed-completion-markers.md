...
an artifact inventory. It also records upstream's `(sample_index,
ranking_score)` rows so request and global summaries can be rebuilt after
worker-local ranking files are discarded. Cache reconciliation trusts a
matching marker without walking sample directories, parsing confidence files,
or recomputing sizes and digests. Directory existence without the marker
remains incomplete.
