...
The Numbered domain section also shows native V/J reference alignments with the
selected reference names and tie counts. Blank operations mean exact matches;
...
V/J junction are not called insertions or mutations.
...
analysis version 3; an update notice blocks calculations against an older API
