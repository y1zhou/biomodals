...
### Job presentation and remote lifecycle

Stage projection, Tool registration, logs and cancellation follow the current
[API Tool service contract](api-tool-service.md) and
[remote-authority ADR](../adr/0007-api-jobs-use-remote-coordinators.md).
The former service-owned GROMACS timeline, direct-call cancellation loop and
CLI log-stream protocol have been removed from this document because they no
longer describe the implementation. Provider logs remain diagnostics, never
