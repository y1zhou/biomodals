...
| Helper layer | `bounded_map` | Small deterministic local concurrency helper | Thread pools are not durable schedulers or global limits |
...
- Before writing an incomplete GROMACS directory, the adapter atomically
  ...
  Run or an explicit Successor may repair incomplete output. Successor lineage
  comes only from immutable launch state, never from the marker's current
  owner, and is recorded even when the Successor only reads a complete cache.
...
Volume-I/O lock. Every kernel operation that can explicitly checkpoint the
workflow Volume acquires this lock before the SQLite writer. Provider RPCs stay
outside both locks. This ordering prevents a commit or reload while another
input has an open Volume file without extending the SQLite writer across
filesystem or workload I/O.
...
active. The unified CLI accepts only positive values when
`--max-gpu-containers` is supplied. CPU-only Runs may still use a zero GPU
ceiling through workload defaults when the CLI option is omitted.
