...
def af3score_prepare(
    ...
    for pdb in all_files:
        ...
        if (
            output_dir / stem / "seed-10_sample-0" / "summary_confidences.json"
        ).exists() and (
            output_dir / stem / "seed-10_sample-0" / "confidences.json"
        ).exists():
    ...
    for source_path in pending_files:
        shutil.copy2(
            metrics_input_dir / f"{source_path.stem.casefold()}.pdb",
    ...
    for batch_json_dir in sorted(
        path for path in (batch_dir / "json").iterdir() if path.is_dir()
    ):
        chunk_specs.append(
            {
                ...
                "batch_pdb_dir": str(
                    (batch_dir / "pdb") / batch_json_dir.name
                ),  # For `02_prepare_pdb2jax.py`.
            }
        )
...
def af3score_postprocess(
    ...
    all_files = sorted(
        input_files, key=str.casefold
    )  # Reuse the prepared input file list.
    ...
    for path in (paths["output_dir"], paths["failed_dir"], paths["metrics_input_dir"]):
        ...
        )  # Ensure required postprocess dirs exist.
    ...
    for candidate in sorted(
        paths["output_dir"].iterdir(), key=lambda path: path.name.casefold()
    ):
        ...
        if not (summary_confidences.exists() and confidences.exists()):
            continue
        (paths["metrics_view_dir"] / candidate.name).symlink_to(
...
def submit_af3score_task(
    ...
    if input_root.is_file():
        all_files = (
            [input_root]
            if input_root.suffix.lower() in SUPPORTED_INPUT_SUFFIXES
            else []
        )
    else:
        all_files = [
