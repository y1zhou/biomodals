    ...
    def _recover_publications(self) -> None:
        ...
        for node in repository.list_nodes(self.execution_run_id):
            if (
                ...
                and node.node_key
                not in {PREPARE_NODE, BOLTZ_DOWNLOAD_NODE, CHAI_DOWNLOAD_NODE}
            ):
                ...
                for task_key in call.task_keys:
                    ...
                    if observation == AvailabilityStatus.MISSING:
                        repository.fail_task(
                            ...
                            call.node_key,
                            task_key,
                            message="ABCFold2 returned without a valid publication",
                            now=self._now(),
                        )
                    else:
                        repository.record_task_result_observation(
            ...
            with self.store.transaction():
                ...
                for record in records:
                    observation = (
                        ...
                        if node_key
                        in {PREPARE_NODE, BOLTZ_DOWNLOAD_NODE, CHAI_DOWNLOAD_NODE}
