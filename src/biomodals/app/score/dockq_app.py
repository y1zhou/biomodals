...
def build_local_output_path(
    ...
    *,
    ...
    clean_run_name = _clean_filename_part(run_name)
    if not clean_run_name:
        raise ValueError("run_name must be non-empty")
    clean_extension = re.sub(r"[^A-Za-z0-9._-]+", "_", extension.strip())
    clean_extension = clean_extension.replace("/", "_").replace("\\", "_").strip("_")
    if clean_extension and not clean_extension.startswith("."):
        clean_extension = f".{clean_extension}"
    out_path = resolve_local_output_dir(out_dir) / f"{clean_run_name}{clean_extension}"
