...
class OligoformerExecutionRuntime:
    ...
    def _recover_publications(self) -> None:
        ...
        for node in repository.list_nodes(self.execution_run_id):
            if (
                ...
                and node.node_key not in self._plan_nodes
            ):
                ...
                for task_key in call.task_keys:
                    ...
                    if observation == AvailabilityStatus.MISSING:
                        repository.fail_task(
                            ...
                            call.node_key,
                            task_key,
                            message="OligoFormer returned without a valid publication",
                            now=self._now(),
                        )
                    else:
                        repository.record_task_result_observation(
            ...
            with self.store.transaction():
                ...
                for record in records:
                    observation = (
                        ...
                        if node_key in self._plan_nodes
                        or node_key == PITA_REFERENCE_NODE
    ...
    def _admit_remote_tasks(self, required: set[str]) -> None:
        ...
        candidates = form_fixed_batches(tuple(descriptors))
        ordered = select_admissible_candidates(
            candidates,
            available_total_slots=len(candidates),
