"""Nanobody humanization request staging and verified scientific result presentation."""
...
from biomodals.workflow.nanobody_humanization.preparation import preparation_digest
from biomodals.workflow.nanobody_humanization.workflow import OUT_VOLUME_NAME

NANOBODY_ARCHIVE_SCHEMA = "nanobody_humanization/1"
MAX_MANIFEST_BYTES = 16 * 1024 * 1024


class NanobodyToolAdapter:
    ...
    def _prepare_environment(self, job: JobRecord) -> SubmissionWait | None:
        ...
        if not task.done():
            return SubmissionWait(
                ...
                message="Preparing and verifying nanobody models before execution",
            )
        try:
            task.result()
        except Exception as error:
            raise EnvironmentPreparationError(
                "Nanobody humanization models could not be prepared; operator intervention is required"
            ) from error
        return None
    ...
    async def _stage_models(deployment: DeploymentIdentity) -> None:
        ...
        for name in ("stage_hudiff_nb_models", "stage_abnativ2_vhh_models"):
    ...
    async def prepare_result(
        ...
        *,
        ...
        try:
            ...
            manifest = NanobodyManifest.model_validate_json(content)
            if (
                ...
                or manifest.preparation_digest != preparation_digest(request.parents)
            ):
                raise ValueError(
                    "Nanobody humanization publication does not match its request"
                )
        except ValueError as exc:
            raise ArtifactIntegrityError(
                "Nanobody humanization publication manifest is invalid"
            ) from exc
        ...
        try:
            with TemporaryDirectory(
                ...
                try:
                    ...
                    raise ArtifactIntegrityError(
                        "Nanobody humanization scientific files failed verification"
                    ) from exc
        ...
        return PreparedResult(
            ...
            size_bytes=built.size_bytes,
            sha256=built.sha256,
            archive_schema=NANOBODY_ARCHIVE_SCHEMA,
        )
